...
from backend.app.schemas.investment_planning import (
    DensificationCreate,
    DensificationSchema,
...
@router.post(
    "/", response_model=DensificationSchema, status_code=status.HTTP_201_CREATED
)
async def create_densification_request(
    ...
    if not current_user.tenant_id:
        raise HTTPException(
            status_code=403,
            detail="No tenant bound to session. Please select a tenant.",
        )
    tenant = current_user.tenant_id
    dup_result = await db.execute(
        select(DensificationRequestORM)
        .where(
...
async def list_densification_requests(
    ...
    query = (
        select(DensificationRequestORM)
        .where(DensificationRequestORM.tenant_id == current_user.tenant_id)
...
async def get_investment_plan(
    ...
    if not plan:
        raise HTTPException(
            status_code=404, detail="Plan not found or still processing"
        )
