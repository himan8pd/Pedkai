...
class LLMService:
    ...
    async def _compute_confidence(
        ...
        try:
            ...
            stats = await repo.get_calibration_stats(
                bin_memory, bin_causal, session=session
            )
            ...
            if stats["total_votes"] >= 50:
                ...
                logger.info(
                    f"Using calibrated confidence: {calibrated_score:.2f} (based on {stats['total_votes']} votes in bin {bin_memory}/{bin_causal})"
                )
    ...
    def _format_incident_context(self, context: Dict[str, Any]) -> str:
        ...
        entity_info = (
            f"Entity: {context.get('entity_name')} ({context.get('entity_type')})"
        )
        upstream = "\n".join(
            [
                f"- {u['entity_type']} {u['entity_name']}"
                for u in context.get("upstream_dependencies", [])
            ]
        )
        downstream = "\n".join(
            [
                f"- {d['entity_type']} {d['entity_name']}"
                for d in context.get("downstream_impacts", [])
            ]
        )
        slas = "\n".join(
            [f"- SLA: {s['entity_name']}" for s in context.get("critical_slas", [])]
        )

        return f"{entity_info}\n\nUpstream Dependencies:\n{upstream}\n\nDownstream Impacts:\n{downstream}\n\nCritical SLAs:\n{slas}"

    async def _format_similar_decisions(
        self, decisions: List[Any], session: Optional[AsyncSession] = None
    ) -> str:
    ...
    async def generate_sitrep(
        ...
        expert_matches = causal_lib.match_causal_templates(
            incident_context.get("anomalies", [])
        )

        context_str = self._format_incident_context(incident_context)
        memory_str = await self._format_similar_decisions(
            similar_decisions, session=session
        )
        ...
        if causal_evidence:
            ...
            for c in causal_evidence:
                ...
                causal_lines.append(
                    f"- **{cause}** Granger-causes **{effect}** (p-value: {p_val}, lag: {lag} periods)"
                )
            causal_str = "\n".join(causal_lines)
        ...
        if expert_matches:
            ...
            for m in expert_matches:
                expert_lines.append(
                    f"- {m['description']} (Expert Confidence: {m['confidence']})"
                )
            causal_str += "\n" + "\n".join(expert_lines)
        ...
        try:
            ...
            cumulative_revenue_loss = (
                await bss_service.calculate_cumulative_active_risk(session=session)
            )

            if customer_ids:
                predicted_revenue_loss = await bss_service.calculate_revenue_at_risk(
                    customer_ids, session=session
                )
                bss_resolved = True
                ...
                for cid in customer_ids:
                    account = await bss_service.get_account_by_customer_id(
                        cid, session=session
                    )
                    if (
                        account
                        and account.service_plan
                        and account.service_plan.tier == "GOLD"
                    ):
                        found_gold = True
                        break
        ...
        exclusion_threshold = engine.get_parameter(
            "sampling_exclusion_revenue_threshold", 5000
        )
        should_bypass_sampling = (
            predicted_revenue_loss > exclusion_threshold if bss_resolved else False
        )

        if not should_bypass_sampling and random.random() > self.sampling_rate:
            logger.info(
                f"Skipping LLM generation due to sampling ({self.sampling_rate}). Revenue ${predicted_revenue_loss} < ${exclusion_threshold}."
            )
        ...
        if not policy_decision.allowed:
            ...
            policy_section += (
                f"Required Actions: {', '.join(policy_decision.required_actions)}"
            )
        ...
        rca_context = incident_context.get(
            "rca_results",
            "Generic anomaly detection triggered (RCA pending graph traversal)",
        )
        ...
        allowed, scrubbed_prompt, reason = sov_service.enforce_data_sovereignty(
            prompt,
            tenant_id=incident_context.get("tenant_id", _settings.default_tenant_id),
        ...
        try:
            ...
            llm_text = (
                llm_resp.text if hasattr(llm_resp, "text") else str(llm_resp)
            ) + policy_section
            model_version = (
                llm_resp.model_version
                if hasattr(llm_resp, "model_version")
                else "unknown"
            )
            prompt_hash = (
                llm_resp.prompt_hash if hasattr(llm_resp, "prompt_hash") else ""
            )

            # Task 3.2: Confidence scoring
            confidence = await self._compute_confidence(
                llm_text, decision_memory_hits, causal_evidence_count, session=session
            )
