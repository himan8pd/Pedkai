...
async def test_full_platform_integration(test_db):
    ...
    async with test_db() as session:
        ...
        assert len(incidents_created) >= 5, (
            f"Expected ≥5 incidents from 50 alarms, got {len(incidents_created)}"
        )
        assert len(incidents_created) <= 20, (
            f"Expected ≤20 incidents, got {len(incidents_created)}"
        )
        print(
            f"✓ STEP 2 PASS: 50 alarms correlated into {len(incidents_created)} incidents"
        )
        ...
        print(
            f"✓ STEP 3 PASS: Sleeping cell detected with severity={sleeping_cell_incident.severity}"
        )
        ...
        assert causal_match is not None, (
            "SITREP not generated or causal analysis missing"
        )
        assert causal_match.get("confidence", 0) >= 0.6, (
            f"Causal confidence too low: {causal_match.get('confidence', 0)}"
        )
        print(
            f"✓ STEP 4 PASS: SITREP generated with causal template (confidence={causal_match.get('confidence')})"
        )
        ...
        assert incidents_prevented_count >= 0, (
            "incidents_prevented count should be non-negative"
        )
        ...
        print(
            f"✓ STEP 6 PASS: ROI Dashboard verified "
        ...
        for incident in incidents_created[:5]:  # Check first 5
            ...
            assert len(audit_trail) >= 2, (
                f"Audit trail too short: {len(audit_trail)} entries, expected ≥2"
            )
...
async def _generate_correlated_alarms(
    ...
    for i in range(count):
        ...
        alarms.append(
            {
                "alarm_id": f"alarm-{i}",
                "alarm_type": ["high_latency", "low_throughput", "high_error_rate"][
                    i % 3
                ],
                "entity_id": entity_id,
            ...
            await session.execute(
                select(func.count(IncidentORM.id)).where(
                    ...
                    & (
                        ...
                        >= incident.created_at - timedelta(hours=1)
                    )
                )
            )
        ).scalar() or 0
