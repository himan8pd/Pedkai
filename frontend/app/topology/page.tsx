...
export default function TopologyPage() {
  ...
  useEffect(() => {
    async function fetchTopology() {
      try {
        // Try without auth first (topology may require token)
        const res = await fetch(`${API_BASE_URL}/api/v1/topology/casinolimit`)
  ...
  useEffect(() => {
    ...
    Object.entries(positions).forEach(([id, pos]) => {
      ...
      if (zoom >= 0.6) {
        ...
        ctx.fillText(ent.name.slice(0, 14), pos.x, pos.y + r + 14)
      }
    })
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center min-h-[60vh]">
        <div className="text-gray-400 animate-pulse text-lg">Loading topology…</div>
      </div>
    )
  }
    ...
    <div className="flex h-[calc(100vh-64px)]">
      ...
      <div className="w-72 border-r border-gray-800 bg-gray-900/50 flex flex-col overflow-hidden">
        ...
        <div className="px-4 py-2 border-b border-gray-800">
          <p className="text-[10px] uppercase tracking-wider text-gray-500 font-bold mb-1">Types</p>
        ...
        <div className="flex-1 overflow-y-auto">
          {filteredEntities.slice(0, 200).map((ent) => (
            ...
            >
              ...
              <span className="text-[10px] text-gray-600 pl-4">{ent.entity_type}</span>
            </button>
          ))}
        ...
        {selectedEntity && (
          <div className="absolute bottom-4 left-4 right-4 max-w-md bg-gray-900 border border-gray-700 rounded-xl p-4 shadow-2xl">
            ...
            <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-sm">
              ...
              <span className="text-gray-200">{selectedEntity.entity_type}</span>
