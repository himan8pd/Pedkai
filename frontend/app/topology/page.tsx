...
import { ZoomIn, ZoomOut, Maximize2, Search, Layers, Network, Map as MapIcon, GitGraph, Sparkles, History } from "lucide-react";
...
export default function TopologyPage() {
  ...
  const [searchScope, setSearchScope] = useState<"all" | "cmdb" | "operational">("all");
  ...
  // Recent entities (persisted per tenant in localStorage)
    ...
    const delay = setTimeout(async () => {
      ...
      try {
        const res = await authFetch(
          `/api/v1/topology/${encodeURIComponent(tenantId)}/search?q=${encodeURIComponent(searchQuery)}&scope=${searchScope}`,
        );
        if (res.ok) {
  ...
  }, [searchQuery, searchScope, tenantId, token]);
  ...
  // 1d. Recent entities — load per-tenant from localStorage
  const recentsKey = tenantId ? `pedkai:topology:recents:${tenantId}` : null;
  useEffect(() => {
      ...
      <div className="w-80 border-r border-cyan-900/40 bg-[#0a2d4a] flex flex-col z-10">
        <div className="p-4 border-b border-cyan-900/40 space-y-4 shadow-sm bg-[#06203b]">
          ...
          <div className="relative">
            ...
            {/* Search scope — search CMDB vs Operational separately to prove Dark Nodes */}
            <div className="mt-2">
              <p className="text-[10px] uppercase tracking-wider text-white/50 font-bold mb-1">Search in</p>
              <div className="flex rounded-lg overflow-hidden border border-cyan-900/50 text-[10px]">
                {(["all", "cmdb", "operational"] as const).map((val) => (
                  <button
                    key={val}
                    onClick={() => setSearchScope(val)}
                    className={`flex-1 py-1 font-semibold capitalize transition-colors ${
                      searchScope === val
                        ? "bg-cyan-400 text-gray-950"
                        : "bg-[#0a2d4a] text-white/70 hover:text-white hover:bg-[#0d3b5e]"
                    }`}
                  >
                    {val === "all" ? "All" : val === "cmdb" ? "CMDB" : "Operational"}
                  </button>
                ))}
              </div>
            </div>
            ...
            {isSearching ? (
              ...
              <div className="divide-y divide-cyan-900/20">
                {searchResults.map(res => (
                  <div key={res.id} className="flex items-stretch hover:bg-white/5 transition-colors group">
                    <button
                      onClick={() => { pushRecent(res); setSeedId(res.id); setSearchQuery(""); }}
              ...
              <div className="p-4 text-xs text-center space-y-1">
                {searchScope === "cmdb" ? (
                  <>
                    <p className="text-amber-300 font-semibold">Not declared in CMDB</p>
        ...
        {/* Recent entities (empty-state helper) */}
        {!searchQuery && !seedId && recents.length > 0 && (
                ...
                <button
                  onClick={() => {
                    pushRecent({
                      ...
                      external_id: selectedEntity.external_id ?? selectedEntity.id,
                    });
                    setInvestigateEntity(selectedEntity);
                  }}
