...
```mermaid
graph LR
    RAW[Raw Evidence<br/>Ticket / Alarm / Telemetry] 
    --> S1[Step 1<br/>Entity Resolution]
    --> S2[Step 2<br/>Operational Fingerprinting]
    --> S3[Step 3<br/>Failure Mode Classification]
    --> S4[Step 4<br/>Temporal-Semantic Embedding]
    --> STORE[Fragment Store<br/>pgvector + PostgreSQL]
    
    ST[Shadow Topology] -.->|"2-hop expansion"| S1
    ITSM[ITSM Change Data] -.->|"Change window proximity"| S2
    VND[Vendor Upgrade Calendar] -.->|"Upgrade cycle data"| S2
    KPI[Traffic Profiles] -.->|"Diurnal patterns"| S2
    TAX[Dark Graph Taxonomy] -.->|"Divergence type signatures"| S3
...
Cold fragments can still participate in snap evaluation, but through a two-stage process:

1. **Metadata match**: When targeted retrieval (Stage 1) finds entity overlap with a cold fragment (via the PostgreSQL metadata lookup table), flag the cold fragment as a retrieval candidate.

2. **Embedding comparison**: Retrieve the cold fragment's embedding from Parquet and compute similarity against the new fragment. If the score exceeds the affinity threshold, retrieve the full compressed record.

3. **Cold snap bonus**: If a cold fragment snaps (evidence separated by 6+ months connects), flag the resulting hypothesis with elevated significance — `COLD_SNAP` tag. These discoveries are exceptionally high-value: evidence that sat dormant for half a year suddenly illuminating a network truth that no other system could find. Route immediately for operator review with explicit commentary on the time span.
