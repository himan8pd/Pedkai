...
**Revision:** V8.2 — Full Topology Reconciliation (Nodes + Edges + Identities)

---

## Preamble: The Dark Graph — Full Topology Reconciliation

Pedkai's "Dark Graph" is not one phenomenon. It is not even limited to one kind of topology element. Previous iterations of this document focused narrowly on discovering unknown **edges** between known nodes — connections the CMDB failed to record. This is one manifestation, but far from the only one. In real telecom networks, the CMDB is wrong about the **nodes themselves**, about their **identities**, about their **attributes**, and about their **relationships**. Pedkai reconciles **all** of these discrepancies.
...
**Dark Edges** — Connections between entities (whether known or dark) that the CMDB does not record. This was the original and sole focus of previous iterations.
...
### Why the Dark Graph Exists — Four Causal Types

The topology elements above (dark nodes, phantom nodes, identity mutations, dark edges, phantom edges, dark attributes) arise from four fundamentally different causes. Each requires different evidence strategies, but all feed the same hypothesis engine.
...
### Dark Graph Type 4: Multi-Party Handover & Cross-Lifecycle Identity Fragmentation

This is the most pervasive and commercially devastating dark graph — and the hardest to solve. It arises not from malicious actors, not from individual operator behaviour, and not from security policy. It arises from the **fundamental structure of how large infrastructure is built, operated, and evolved across decades by dozens of independent parties**.

Consider deploying a single new radio site on the edge of a motorway. This is not a one-team affair. It is a multi-party, multi-month orchestration involving:

- **Surveyors** — identify coverage not-spots, perform desktop surveys, shortlist candidate locations
- **Radio planners** — model propagation, specify equipment requirements, define frequency/power/tilt parameters
- **Health & Safety assessors** — evaluate site practicality, produce risk assessments, define access protocols
- **Legal teams** — negotiate wayleaves, landlord agreements, planning permissions
- **Small works contractors** — pour foundations, build concrete bases, erect towers
- **Ducting and plumbing contractors** — lay cable routes, install containment
- **Electrical contractors** — wire mains power, install distribution boards, commission generators
- **HVAC contractors** — install cooling and environmental control for equipment cabinets
- **Physical security contractors** — install fencing, gates, locks, CCTV
- **Electronic security contractors** — configure access control systems, alarm monitoring
- **Transmission engineers** — provision backhaul links (fibre, microwave, satellite)
- **Power engineers** — commission UPS systems, battery backup, solar where applicable
- **Public coordination and traffic management** — manage road closures, community liaison
- **Deployment teams** — install radio equipment, configure antennas, commission cells
- **Project Managers** — orchestrate all of the above, control costs and schedules

Each party generates documentation: survey reports, H&S assessments, method statements, design packs, commissioning certificates, as-built drawings, test results, handover packs. This documentation is critical for regulatory compliance, health and safety, and later operational requirements.

**The Identity Fragmentation Problem:**

Every party involved allocates their own identifier to the site. The surveyor's candidate reference (`SURV-M1-J23-004`). The radio planner's cell ID (`LTE-8842-A`). The project manager's work order (`WO-2024-NW-1847`). The landlord's property reference. The electrical contractor's job number. The tower company's structure ID. The planning authority's application reference. When a second operator co-locates on the same site — sharing the physical perimeter or even the same tower — they bring their own complete set of identifiers.

You cannot eliminate these identifiers. Each is the primary key in that party's system. But when documents are exchanged between parties, if everyone insists on using only their own identifier, nobody else understands what site is being discussed. Project managers define formal information exchange processes to mitigate this, but it is **impossible** to record every identifier on every document, every email, every artefact generated across a 6–12 month deployment.

During deployment, this works. The humans involved know intimately which identifiers map to which physical location. They hold the complete cross-reference in their heads.

**The Handover Abyss:**

When the site goes live, it is handed over to Operations. The operations team does not need — and does not receive — the full deployment archive. They want:
- The site identifier (in *their* system)
- Health & Safety access instructions
- Equipment inventory and serial numbers
- Contact details for site access booking
- System records linking the site to the wider network

Only a fraction of the deployment documentation makes it into the CMDB. The rest — survey reports, method statements, as-built drawings, commissioning certificates, the complete identity cross-reference — is archived in systems like **Salesforce Sitetracker**, **SharePoint project libraries**, **Aconex**, or simply people's **email inboxes**.

Months later, an obscure but critical question arises: Does cabinet 3 on site NW-1847 contain only our equipment, or is the co-locating operator's fibre also routed through it? The answer exists — in a photograph attached to a handover email from the deployment contractor, cross-referenced by a job number that appears in a Sitetracker milestone, linked to a planning application that names the co-locator. No single system holds the answer. No single person remembers. The dark graph.

**The Cross-Lifecycle Compounding:**

This pattern does not occur once. It compounds across technology generations. A site originally deployed for 3G fifteen years ago — with a 3G-era master site identifier — is now hosting 4G and 5G equipment. The 3G technology is being decommissioned across the entire network. But you cannot delete the site record using the 3G identifier because the 4G and 5G equipment is referenced by it. The 3G decommissioning programme doesn't have visibility into which sites also carry 4G/5G. The 4G/5G deployment programme used a different site identifier altogether. The CMDB has two records that are actually one physical location, with different technology stacks, different identifiers, different responsible teams, and no explicit link between them.

Multiply this by every core network augmentation, every data centre migration, every RAN refresh, every transmission upgrade across a 20-year operational history. The result: **dark graphs everywhere**. Dark identifiers (same site, different names). Dark provenance (deployment details lost at handover). Dark attributes (physical configuration facts buried in archived documents). Dark edges (cross-party dependencies that no single system records). Phantom identifiers (decommissioned technology labels still polluting active records).

**Why this is the hardest use case:**

Types 1–3 deal with structured or semi-structured data: telemetry flows, CMDB records, ticket fields. Type 4 deals with **largely unstructured data**: PDF survey reports, email threads, photograph metadata, SharePoint folder hierarchies, Sitetracker custom fields, planning application documents, landlord correspondence. Each organisation uses different systems, different naming conventions, different document templates. There is no universal schema.

Pedkai must:
1. **Ingest** documents from heterogeneous sources (Sitetracker exports, SharePoint crawls, email PST archives, scanned PDFs with OCR)
2. **Extract** entity identifiers — site codes, job numbers, cell IDs, serial numbers, addresses, coordinates — from unstructured text
3. **Cluster** identifiers that refer to the same physical entity using multi-signal correlation: geographic proximity (coordinates within 50m), naming pattern similarity, temporal co-occurrence in documents, shared party references
4. **Build** an Identity Resolution Graph where each physical site is a node and each identifier is an alias, with provenance tracing every alias back to its source document
5. **Reconcile** this graph against the CMDB — surfacing dark identifiers (aliases the CMDB doesn't know), dark provenance (deployment facts not in operational records), and phantom identifiers (decommissioned labels still active)
6. **Surface** cross-lifecycle links: "Site NW-1847 (your 5G record) is physically the same location as SITE-3G-0442 (your legacy 3G record being decommissioned). Decommissioning 3G here must not affect 5G."

This is where the value is. If it were easy, someone would have done it already. The difficulty is the moat.

**Data availability:** NONE for development. Real deployment archives (Sitetracker exports, SharePoint crawls, handover packs) are not available. However, the **CasinoLimit campaigns can be reinterpreted as multi-party deployment lifecycles**: each campaign instance is a "site deployment" with multiple phases (survey → access → configuration → commissioning), each zone is a "contractor workstream" generating its own identifiers, and the campaign steps are "handover milestones" where context is transferred between parties. We can synthesise deployment-like document artefacts from CasinoLimit data to prove the identity resolution mathematics (see §1.9).
...
- **What axis** (topology element): Dark nodes, phantom nodes, identity mutations, dark edges, phantom edges, dark attributes, dark identifiers — all pass through the same hypothesis lifecycle (CANDIDATE → CORROBORATED → ACCEPTED → INTEGRATED).
- **Why axis** (causal type): Intrusion, tribal knowledge, secure infrastructure, multi-party handover — all contribute evidence to the same hypothesis, weighted by source type.

The noisy-OR confidence model, the multi-modal corroboration requirement, the cross-examination, and the abeyance memory all work identically whether the hypothesis is "this edge exists" or "this node exists but the CMDB doesn't know about it" or "this CI's physical identity has changed" or "these two identifiers refer to the same physical site." The only differences are the **source type weight**, the **policy gate** applied at evidence intake, and the **hypothesis type** guiding what corroboration to seek.

A `NodeHypothesis` (dark node) requires evidence that the entity is real and persistent, not a transient burst. An `IdentityMutationHypothesis` requires evidence linking the old and new identities. An `EdgeHypothesis` requires evidence that both endpoints exist and communicate. An `IdentityFragmentationHypothesis` requires evidence that two or more identifiers from different source systems refer to the same physical entity. All four use the same mathematical framework.

This matters commercially: Pedkai can be sold as "operational reconciliation that captures the real state of your estate" — detecting undocumented VNFs, flagging phantom CIs that waste licence fees, linking emergency hardware swaps to their predecessors, and resolving the identity fragmentation that accumulates across decades of multi-party infrastructure deployment — rather than "cybersecurity threat detection." The security use case demonstrates the engine's power; the operational and lifecycle reconciliation use cases pay the bills.

---

## Preamble B: The Datagerry/CasinoLimit Nexus

Before a single line of code is written, the engineering team must internalise why these two datasets are the perfect proving ground for **all four** Dark Graph types.
...
*Thesis 5 — Cross-Lifecycle Identity Fragmentation (Type 4):* Each CasinoLimit campaign instance has a single canonical name (`aeriella`, `belinda`, etc.) but we synthetically assign parallel identifiers: a "surveyor reference" (`SURV-{inst}-001`), a "contractor job number" (`CW-{hash}`), a "radio planner cell ID" (`CELL-{zone}-{inst}`), and a "co-locator reference" (`COLOC-{inst}-B`). These identifiers appear in synthetic deployment documents (survey reports, H&S assessments, handover packs) generated from CasinoLimit campaign metadata. The CMDB records only the canonical hostname. Pedkai must: (a) extract identifiers from unstructured synthetic documents, (b) cluster identifiers that refer to the same physical instance using geographic, temporal, and party-reference correlation, (c) build an Identity Resolution Graph linking all aliases to a single physical entity, and (d) surface dark identifiers the CMDB doesn't know about — including cross-lifecycle links where a "legacy" instance identifier (simulating 3G-era) is still active in the system while a "current" identifier (simulating 5G) refers to the same location. This thesis maps 1:1 to the real-world multi-party site deployment scenario where surveyors, contractors, operators, and co-locators each maintain independent identifiers for the same physical site.

All five theses are proved using the **same mathematical engine**. CasinoLimit is the crucible.
...
| P1.14 | Synthetic deployment document generator produces ≥20 document sets with multi-party identifiers | Assert ≥20 instances have survey reports, H&S assessments, contractor job sheets, and handover packs with distinct identifier schemes |
| P1.15 | Identity extraction from synthetic documents achieves ≥85% recall | Compare extracted identifiers against known ground truth (planted identifiers per document) |
| P1.16 | Identity clustering correctly groups ≥90% of synonymous identifiers | For each instance, assert all planted identifiers cluster to the same physical entity |
| P1.17 | Cross-lifecycle link detection identifies ≥80% of legacy→current identifier pairs | Inject legacy 3G-era identifiers alongside current 5G identifiers for 20 instances → assert ≥16 correctly linked pairs |
...
### 1.9 — Multi-Party Handover & Cross-Lifecycle Document Synthesis from CasinoLimit (Week 4)

Type 4 Dark Graph data does not exist in CasinoLimit. We synthesise it by reinterpreting CasinoLimit campaigns as multi-party infrastructure deployment lifecycles — structurally identical to how a new radio site is built, documented, and handed to operations.

**The Reinterpretation:**

| CasinoLimit Reality | Deployment Lifecycle Reinterpretation |
|---|---|
| Campaign instance (`aeriella`) | Physical site being deployed (`SITE-NW-1847`) |
| start zone (entry point) | Survey and planning phase — the first party to touch the site |
| bastion zone (network hop) | Civil works and contractor access — the build phase |
| meetingcam zone (application) | Equipment installation and commissioning |
| intranet zone (data store) | Operational handover — the data that reaches (or doesn't reach) the CMDB |
| Campaign steps in `steps.csv` | Deployment milestones where handover between parties occurs |
| Zone-to-zone traversal | Information handover between contractor workstreams |

**Synthetic Deployment Document Generator:**

For each CasinoLimit campaign instance, generate a set of synthetic deployment artefacts that mirror real-world site deployment documentation:

```python
@dataclass
class SyntheticDeploymentDocument:
    document_id: str
    instance_id: str                    # CasinoLimit instance
    document_type: str                  # 'survey_report' | 'h_and_s_assessment' | 'method_statement' |
                                        # 'design_pack' | 'commissioning_cert' | 'handover_pack' |
                                        # 'contractor_job_sheet' | 'planning_application'
    authoring_party: str                # 'surveyor' | 'radio_planner' | 'contractor' | 'h_and_s' |
                                        # 'electrical' | 'transmission' | 'project_manager' | 'co_locator'
    party_identifier: str               # The identifier THIS party uses for the site
    other_identifiers_mentioned: list[str]  # Other parties' identifiers that happen to appear in this doc
    canonical_identifier: str           # The CMDB hostname (ground truth, NOT in doc explicitly)
    created_at: datetime
    content_text: str                   # Synthetic unstructured text body
    embedded_entities: list[dict]       # Entities mentioned: {name, type, context}
    geographic_coordinates: tuple[float, float] | None  # Lat/lon when applicable
    cross_lifecycle_refs: list[str]     # References to older technology-era identifiers
```

**Identifier Scheme Generation:**

For each instance, generate parallel identifiers from different parties, each following a realistic naming convention:

```python
def generate_party_identifiers(instance_id: str) -> dict[str, str]:
    """Generate per-party identifiers for a single physical site/instance."""
    inst_hash = hashlib.md5(instance_id.encode()).hexdigest()[:6]
    return {
        'surveyor':       f"SURV-M1-{inst_hash[:3].upper()}-{int(inst_hash[3:], 16) % 999:03d}",
        'radio_planner':  f"LTE-{int(inst_hash, 16) % 9999:04d}-A",
        'project_manager': f"WO-2024-NW-{int(inst_hash, 16) % 9999:04d}",
        'contractor':     f"CW-{inst_hash.upper()}",
        'electrical':     f"ELEC-JOB-{int(inst_hash, 16) % 99999:05d}",
        'tower_company':  f"TWR-{inst_hash[:4].upper()}-{int(inst_hash[4:], 16) % 99:02d}",
        'co_locator':     f"COLOC-{instance_id[:3].upper()}-B",
        'legacy_3g':      f"3G-SITE-{int(inst_hash, 16) % 9999:04d}",  # Cross-lifecycle: old tech ID
        'current_5g':     f"5G-NR-{int(inst_hash, 16) % 9999:04d}",    # Cross-lifecycle: current tech ID
    }
```

**Document Content Generation Rules:**

1. Each document is authored by exactly one party and uses that party's identifier as its primary site reference
2. Documents may incidentally mention 1–2 other parties' identifiers (simulating cross-references in real handover packs)
3. The CMDB canonical hostname (`start.aeriella.local`) appears in at most 1 document (the final handover pack) — simulating the information loss at the deployment→operations handover
4. Geographic coordinates (derived from instance hash) are consistent across all documents for the same instance but expressed differently (decimal degrees in survey reports, grid references in method statements)
5. Deployment timestamps are spread across weeks, simulating the real timeline of a multi-month site build
6. Cross-lifecycle references: for a subset of instances (e.g., 20), the `legacy_3g` identifier appears in early documents, and the `current_5g` identifier appears in later documents, with the two linked only by geographic proximity and a single oblique reference in a project manager's handover note

**Synthetic Document Corpus Example (instance `aeriella`):**

| Document Type | Authoring Party | Primary Identifier | Other IDs Mentioned | Cross-Lifecycle |
|---|---|---|---|---|
| Survey Report | Surveyor | `SURV-M1-A3F-042` | None | — |
| H&S Assessment | H&S Assessor | `SURV-M1-A3F-042` | `WO-2024-NW-3847` | — |
| Method Statement | Contractor | `CW-A3F7B2` | `SURV-M1-A3F-042` | — |
| Design Pack | Radio Planner | `LTE-3847-A` | `CW-A3F7B2` | `3G-SITE-3847` (legacy predecessor) |
| Commissioning Cert | Deployment Team | `LTE-3847-A` | `TWR-A3F7-04` | — |
| Handover Pack | Project Manager | `WO-2024-NW-3847` | `LTE-3847-A`, `start.aeriella.local` | `3G-SITE-3847` → `5G-NR-3847` (handover note) |
| Co-locator Notice | Co-locator | `COLOC-AER-B` | `TWR-A3F7-04` | — |

The CMDB knows only `start.aeriella.local`. Eight other identifiers exist for the same physical entity. The CMDB is blind to all of them until the handover pack — and even then, only one additional identifier (`WO-2024-NW-3847`) might make it into the operational record.

**Identity Resolution Algorithm (Pre-Hypothesis):**

Before feeding into the Phase 2 hypothesis engine, the ingestion layer performs coarse identity clustering:

```
for each document in corpus:
    extract identifiers using regex + NER (named entity recognition for site codes, job numbers, grid refs)
    for each identifier_pair co-occurring in the same document:
        add co_occurrence_edge(id_A, id_B, document_id, confidence=0.7)
    for each identifier with geographic coordinates:
        for each other identifier within 50m radius:
            add geographic_proximity_edge(id_A, id_B, distance, confidence=0.6)
    for each identifier pair sharing a common party reference:
        add party_linkage_edge(id_A, id_B, party, confidence=0.5)

identity_clusters = connected_components(co_occurrence_graph, threshold=0.5)

for each cluster:
    emit IdentityFragmentationCandidate(
        canonical_id = best_match_to_cmdb(cluster),  # if any
        alias_ids = cluster.all_identifiers,
        confidence = cluster_cohesion_score,
        provenance = {doc_id: extracted_identifiers for each source document},
        cross_lifecycle_links = detect_technology_era_transitions(cluster)
    )
```

```python
@dataclass(frozen=True)
class IdentityFragmentationCandidate:
    """A cluster of identifiers that likely refer to the same physical entity."""
    candidate_id: str
    canonical_cmdb_id: str | None       # CMDB entity it maps to (None if dark site)
    alias_identifiers: dict[str, str]   # {party_role: identifier}
    geographic_centroid: tuple[float, float] | None
    document_provenance: dict[str, list[str]]  # {doc_id: [identifiers found]}
    cross_lifecycle_links: list[dict]    # [{old_id, new_id, technology_era, evidence}]
    cluster_cohesion: float             # 0.0–1.0: how tightly the identifiers co-occur
    cmdb_coverage: float                # Fraction of identifiers known to CMDB (typically very low)
```

**The Handover Data Loss Metric:**

For each identity cluster, compute the information loss at the deployment→operations boundary:

$$
L_{handover} = 1 - \frac{|\text{identifiers in CMDB}|}{|\text{identifiers in deployment archive}|}
$$

For `aeriella` with 9 deployment identifiers and 1 CMDB identifier: $L_{handover} = 1 - \frac{1}{9} = 0.889$ — the CMDB has lost 88.9% of the site's identity information at handover.

**Storage:** Synthetic deployment documents write to a `deployment_documents` table with JSONB payload for unstructured content. Identity clusters write to `identity_fragment_clusters`. Both are indexed by instance for reconciliation joins. A `cross_lifecycle_links` table specifically tracks old→new technology-era identifier transitions.

**Why this matters:** Every telecom operator has thousands of sites where the CMDB knows one identifier and the deployment archive contains eight more. When a co-locator dispute arises, when a decommissioning programme must know which sites carry multiple technologies, when an operations engineer needs the original H&S assessment for a site visit — the information exists, but nobody can find it because nobody can cross-reference the identifiers. Pedkai builds the cross-reference automatically. This is the use case that makes infrastructure directors say "shut up and take my money" — because they've been burned by this exact problem, repeatedly, for decades, and every manual attempt to fix it has failed because humans cannot maintain cross-references across 50,000 sites and 15 years of documentation. If it were easy, someone would have done it already. The difficulty is the moat.
...
# ─── Hypothesis Type 4: Identity Fragmentation Hypotheses (Multi-Party Handover) ───

@dataclass
class IdentityFragmentationHypothesis:
    """Hypothesis that multiple identifiers from different source systems refer to the same physical entity."""
    hypothesis_id: str
    canonical_entity_id: str | None     # CMDB entity, if one exists in the cluster
    alias_identifiers: dict[str, str]   # {party_role: identifier} — all suspected aliases
    state: HypothesisState
    ...
    # Correlation signals
    geographic_proximity: float         # 0.0–1.0: how close are the coordinates across documents?
    document_co_occurrence: float       # 0.0–1.0: how often do these IDs appear in the same document?
    party_linkage: float                # 0.0–1.0: do shared party references connect the identifiers?
    temporal_overlap: float             # 0.0–1.0: do the identifiers appear in overlapping time windows?
    naming_pattern_coherence: float     # 0.0–1.0: do the identifiers follow recognisable per-party naming conventions?
    
    # Cross-lifecycle
    cross_lifecycle_links: list[dict]   # [{old_id, new_id, old_era, new_era, evidence}]
    technology_eras: set[str]           # {'3G', '4G', '5G'} — which generations are represented
    
    # Coverage metrics
    cmdb_coverage: float                # Fraction of aliases known to CMDB (typically very low)
    handover_loss: float                # 1 - (cmdb_known / total_aliases)
    total_source_documents: int         # How many documents contributed to this cluster
    ...
    # Recommended action
    policy_decision: str | None         # "register_aliases" | "merge_identifiers" | "link_lifecycle" | "review"
```
...
| `document_co_occurrence` (identifiers in same document) | 0.70 | Type 4 | Two identifiers appearing in the same handover document is strong evidence they refer to the same entity |
| `geographic_proximity` (coordinates within 50m) | 0.65 | Type 4 | Two identifiers with near-identical geographic coordinates very likely refer to the same physical site |
| `party_linkage` (shared contractor/party between identifier sources) | 0.50 | Type 4 | Same party references both identifiers, implying knowledge of equivalence |
| `cross_document_chain` (identifier A in doc1 → identifier B in doc2 via shared party/reference) | 0.55 | Type 4 | Transitive linkage across deployment documentation |
| `cross_lifecycle_reference` (old-era ID referenced alongside new-era ID) | 0.60 | Type 4 | Design pack references legacy 3G site ID when specifying 5G equipment — implies same location |
| `naming_pattern_match` (identifier follows known party naming convention) | 0.35 | Type 4 | Weak individually, but helps classify which party an identifier belongs to |
...
| `document_co_occurrence` (identifiers in same deployment doc) | 0.70 | Identity fragmentation | Two identifiers co-occurring in a handover pack = strong same-entity signal |
| `geographic_proximity` (coordinates within 50m across docs) | 0.65 | Identity fragmentation | Near-identical location = likely same physical site |
| `party_linkage` (shared party between identifier sources) | 0.50 | Identity fragmentation | Same contractor references both — implies equivalence |
| `cross_lifecycle_ref` (old-era and new-era ID co-referenced) | 0.60 | Identity fragmentation | Legacy site ID in same context as current — same location, different tech era |
| `cmdb_alias_gap` (identifier exists in docs but not CMDB) | 0.45 | Identity fragmentation | Dark identifier: deployment archive has it, CMDB doesn't |
...
**Note on Type 4 weights:** Identity fragmentation evidence is qualitatively different from Types 1–3 because it operates on **document-extracted identifiers** rather than telemetry or ticket fields. The highest-weight signal is `document_co_occurrence` (0.70) because two identifiers appearing in the same physical document is near-definitive evidence of equivalence — no contractor writes two different site references in one document unless they refer to the same site. Geographic proximity (0.65) is the strongest independent corroboration: if a survey report places `SURV-M1-A3F-042` at coordinates 51.5074°N, 0.1278°W, and a commissioning certificate places `LTE-3847-A` at 51.5074°N, 0.1278°W, the probability they are the same site is extremely high. Cross-lifecycle references (0.60) are particularly valuable commercially because they surface the links between technology generations that decommissioning programmes desperately need. The multi-modal rule still applies: geographic proximity alone cannot promote a hypothesis — it needs document co-occurrence or party linkage as a second source.
...
**Critical Rule:** A hypothesis CANNOT be promoted to `CORROBORATED` with evidence from a single source type, regardless of confidence score. If 100 network flows all say the same thing, that's still single-source. Pedkai requires at least **2 distinct source types** agreeing. This is the anti-hallucination discipline from Vision V8. This rule applies equally to edge, node, identity mutation, and identity fragmentation hypotheses.
...
### 2.3c — The Identity Fragmentation Corroboration Pipeline (Week 7)

Runs after the node and edge corroboration pipelines. For every `IdentityFragmentationCandidate` from Phase 1's §1.9 document synthesis:

```
# ═══ IDENTITY FRAGMENTATION CORROBORATION ═══
for each fragment_cluster in divergence_report.identity_fragments:
    H = IdentityFragmentationHypothesis(
        canonical_entity_id=fragment_cluster.canonical_cmdb_id,
        alias_identifiers=fragment_cluster.alias_identifiers,
        state=CANDIDATE)
    
    # Signal 1: Document co-occurrence (strongest signal)
    # If two identifiers appear in the same physical document, they almost certainly 
    # refer to the same entity — no contractor writes two site references in one 
    # document unless they mean the same site
    co_occurring_pairs = find_identifier_pairs_in_same_document(fragment_cluster)
    for pair in co_occurring_pairs:
        H.add_evidence(EvidenceItem(source_type='document_co_occurrence', weight=0.70,
                                     specificity=0.9, document_id=pair.document_id))
    
    # Signal 2: Geographic proximity
    # Compare coordinates extracted from different documents (survey reports use decimal degrees,
    # method statements use grid references — the normalisation layer converts both)
    coords = extract_coordinates_per_identifier(fragment_cluster)
    for id_pair, distance_m in compute_pairwise_distances(coords):
        if distance_m <= 50:  # Within 50 metres = same site
            H.add_evidence(EvidenceItem(source_type='geographic_proximity', weight=0.65,
                                         specificity=max(0.5, 1.0 - distance_m / 100)))
    
    # Signal 3: Party linkage
    # If the same contractor/party is referenced by both identifiers (even in different docs),
    # they likely know the equivalence
    shared_parties = find_shared_parties(fragment_cluster)
    for party in shared_parties:
        H.add_evidence(EvidenceItem(source_type='party_linkage', weight=0.50,
                                     specificity=0.7, party=party))
    
    # Signal 4: Cross-document chain
    # Identifier A appears in doc1 authored by surveyor. Doc2 (authored by contractor) 
    # references both the surveyor's ID and introduces identifier B. Transitive link.
    chains = find_transitive_chains(fragment_cluster)
    for chain in chains:
        H.add_evidence(EvidenceItem(source_type='cross_document_chain', weight=0.55,
                                     specificity=min(1.0, 1.0 / len(chain.hops)),
                                     chain_length=len(chain.hops)))
    
    # Signal 5: Cross-lifecycle references
    # A design pack references a legacy 3G site ID when specifying 5G equipment.
    # This is gold — it proves the physical site spans technology generations.
    lifecycle_links = detect_technology_era_transitions(fragment_cluster)
    for link in lifecycle_links:
        H.add_evidence(EvidenceItem(source_type='cross_lifecycle_reference', weight=0.60,
                                     specificity=0.85,
                                     old_era=link.old_era, new_era=link.new_era))
        H.cross_lifecycle_links.append(link)
        H.technology_eras.update([link.old_era, link.new_era])
    
    # Compute confidence and coverage metrics
    H.confidence = noisy_or(H.evidence_bundle)
    H.cmdb_coverage = fragment_cluster.cmdb_coverage
    H.handover_loss = 1 - fragment_cluster.cmdb_coverage
    H.total_source_documents = len(fragment_cluster.document_provenance)
    
    # Promote if multi-modal (same rule as all other hypothesis types)
    if has_multiple_source_types(H.evidence_bundle) and H.confidence >= threshold:
        H.state = CORROBORATED
    
    # Determine recommended action
    if H.canonical_entity_id is not None:
        if H.cross_lifecycle_links:
            H.policy_decision = "link_lifecycle"      # Create cross-era links in CMDB
        else:
            H.policy_decision = "register_aliases"     # Add aliases to existing CI
    else:
        H.policy_decision = "review"                   # Dark site: no CMDB entity at all
```

**Worked Example — Instance `aeriella`:**

Hypothesis: Identifiers `SURV-M1-A3F-042`, `LTE-3847-A`, `WO-2024-NW-3847`, `CW-A3F7B2`, `TWR-A3F7-04`, `COLOC-AER-B`, `3G-SITE-3847`, `5G-NR-3847`, `start.aeriella.local` all refer to the same physical entity.

| Evidence | Source Type | $w_i$ | $s_i$ | $1 - w_i \cdot s_i$ |
|---|---|---|---|---|
| `SURV-M1-A3F-042` + `WO-2024-NW-3847` in H&S Assessment | `document_co_occurrence` | 0.70 | 0.9 | 0.37 |
| `CW-A3F7B2` + `SURV-M1-A3F-042` in Method Statement | `document_co_occurrence` | 0.70 | 0.9 | 0.37 |
| Survey and Commissioning coords within 0m | `geographic_proximity` | 0.65 | 1.0 | 0.35 |
| Same contractor (`CW-A3F7B2`) links surveyor to radio planner | `party_linkage` | 0.50 | 0.7 | 0.65 |
| Design Pack mentions `3G-SITE-3847` alongside `LTE-3847-A` | `cross_lifecycle_reference` | 0.60 | 0.85 | 0.49 |

$$C(H) = 1 - (0.37 \times 0.37 \times 0.35 \times 0.65 \times 0.49) = 1 - 0.0153 = 0.985$$

Confidence: **0.985** — from document archaeology alone, without a single byte of telemetry. Five evidence items from four distinct source types. The multi-modal requirement is satisfied.

Handover loss: $L_{handover} = 1 - \frac{1}{9} = 0.889$ — the CMDB knows 11.1% of the site's identity landscape.

**Why this matters commercially:** A CIO sees this and understands immediately: "We have 50,000 sites. Each has an average of 8 identifiers across deployment archives. Our CMDB knows 1.2 of them. Pedkai just showed us the other 6.8 — including the cross-lifecycle links our decommissioning programme has been trying to build manually for 18 months."
...
**Multi-Party Handover Application (Type 4):** A survey report from month 1 of a site deployment mentions `SURV-M1-A3F-042` with coordinates `51.5074°N, 0.1278°W`, but no other identifiers. This goes into abeyance with TTL of 730 days (reflecting the multi-year lifecycle of infrastructure deployment and the decades-long persistence of deployment archives). Eight months later, a contractor's method statement references a job at coordinates `51.5074°N, 0.1279°W` (1m discrepancy — GPS variance) under job number `CW-A3F7B2`. The geographic proximity sweep fires and links the two identifiers. Three years later, a decommissioning programme scans archived design packs and discovers that `3G-SITE-3847` was the original identifier for this location. The abeyance sweep fires again, extending the identity cluster across technology generations. This is the power of very-long-duration abeyance for Type 4 Dark Graph — some identity fragments only become important years after they were created, when a decommissioning decision or a co-locator dispute surfaces the need for the cross-reference that was never formally recorded.
...
| P2.15 | Identity fragmentation hypothesis correctly clusters ≥90% of synonymous identifiers | For 20 instances with synthetic deployment docs → assert IdentityFragmentationHypothesis clusters all party identifiers with confidence ≥0.80 |
| P2.16 | Cross-lifecycle links correctly identified for ≥80% of multi-era instances | For instances with legacy_3g + current_5g identifiers → assert cross_lifecycle_links populated with correct old→new pairing |
| P2.17 | Single-document-only identifiers do NOT create fragmentation hypotheses | Inject an identifier that appears in exactly one document with no geographic or party corroboration → assert hypothesis stays CANDIDATE |
| P2.18 | Handover loss metric correctly computed | For instance with 9 identifiers and 1 in CMDB → assert handover_loss = 0.889 ±0.01 |
| P2.19 | Abeyance memory resolves Type 4 items across long timescales | Inject document with identifier + coordinates, then inject second document 6 months later with different identifier at same coordinates → assert abeyance sweep fires and links them |
...
*Story G: The Deployment Archive (Identity Fragmentation — Type 4)*
Show the identity cluster for instance `aeriella`:
- CMDB knows 1 identifier: `start.aeriella.local`
- Pedkai extracted 8 additional identifiers from synthetic deployment documents:
  - Surveyor: `SURV-M1-A3F-042` (from survey report)
  - Radio Planner: `LTE-3847-A` (from design pack)
  - Project Manager: `WO-2024-NW-3847` (from handover pack)
  - Contractor: `CW-A3F7B2` (from method statement)
  - Tower Company: `TWR-A3F7-04` (from commissioning certificate)
  - Co-locator: `COLOC-AER-B` (from co-locator notice)
  - Legacy 3G: `3G-SITE-3847` (from design pack, cross-lifecycle reference)
  - Current 5G: `5G-NR-3847` (from project manager handover note)
- IdentityFragmentationHypothesis confidence: 0.985
- Evidence: 5 items from 4 source types (document_co_occurrence, geographic_proximity, party_linkage, cross_lifecycle_reference)
- Handover loss: 88.9% — the CMDB lost 8 of 9 identifiers at the deployment→operations boundary
- Cross-lifecycle link: `3G-SITE-3847` → `5G-NR-3847` — same physical site, different technology eras

"Your CMDB knows this site by one name. Nine parties know it by nine different names. The 3G team is decommissioning `3G-SITE-3847` — they have no idea that your live 5G service is at the same location. Pedkai found the link buried in a three-year-old design pack. Nobody told it to look there. It found it because two identifiers appeared in the same document, at the same coordinates, referenced by the same contractor. Document archaeology, not guesswork."

"For clients with 50,000 sites and 15 years of deployment archives in Sitetracker, SharePoint, and email — this is the use case that pays for the entire platform. Every decommissioning programme, every co-locator dispute, every regulatory audit that needs to trace a site back to its original planning permission — the answer is in the archive. Pedkai finds it."
...
- 1 identity fragmentation cluster with 8 dark identifiers ready for CMDB alias registration (aeriella)
- 1 cross-lifecycle link ready for CMDB integration (3G-SITE-3847 → 5G-NR-3847)
- Each with full provenance chain
- "Write to Datagerry" for the consented items

"Click this button. Your CMDB just healed itself. A missing device got registered. A dead asset got flagged. A hardware swap got linked. Eight site identifiers that were scattered across deployment archives are now cross-referenced. A dangerous 3G/5G decommissioning collision was averted. And the knowledge that was trapped in Alice's head is now in the graph."
...
── TYPE 4: IDENTITY FRAGMENTATION (document-archaeology, multi-party) ──

  [F1] Identity Cluster: start.aeriella.local
       Confidence: 0.985 (ACCEPTED)
       CMDB Coverage: 11.1% (1 of 9 identifiers known)
       Handover Loss: 88.9%
       Evidence: 4 source types, 5 items
       ├─ document_co_occurrence: SURV-M1-A3F-042 + WO-2024-NW-3847 in H&S Assessment
       ├─ document_co_occurrence: CW-A3F7B2 + SURV-M1-A3F-042 in Method Statement
       ├─ geographic_proximity:   Survey → Commissioning coords: 0m distance
       ├─ party_linkage:          Contractor CW-A3F7B2 links surveyor to radio planner
       └─ cross_lifecycle_ref:    3G-SITE-3847 → 5G-NR-3847 in Design Pack
       Dark Identifiers:
       ├─ SURV-M1-A3F-042  (surveyor)        │ NOT in CMDB
       ├─ LTE-3847-A       (radio planner)    │ NOT in CMDB
       ├─ WO-2024-NW-3847  (project manager)  │ NOT in CMDB
       ├─ CW-A3F7B2        (contractor)        │ NOT in CMDB
       ├─ TWR-A3F7-04      (tower company)     │ NOT in CMDB
       ├─ COLOC-AER-B      (co-locator)        │ NOT in CMDB
       ├─ 3G-SITE-3847     (legacy 3G)         │ NOT in CMDB  ⚠ CROSS-LIFECYCLE
       └─ 5G-NR-3847       (current 5G)        │ NOT in CMDB  ⚠ CROSS-LIFECYCLE
       ⚠ DECOMMISSION RISK: 3G programme targeting 3G-SITE-3847 will impact live 5G service
       Recommendation: REGISTER ALIASES + LINK LIFECYCLE in CMDB
...
Type 'pedkai integrate --fragment F1 --action register-aliases' to register dark identifiers.
...
| 9b. Story G: Type 4 | 45s | Show identity cluster for aeriella. 9 identifiers, 1 in CMDB. | "9 names for 1 site. Your CMDB knows 1. The 3G decom will break 5G." |
...
| 12. Run `pedkai integrate` | 10s | Write nodes + edges + aliases to Datagerry. | "Dark node registered. Phantom flagged. Edges created. Aliases linked." |
| 13. Refresh Datagerry | 15s | Browse CMDB. New CIs, relationships, and aliases visible. | "Your CMDB just healed itself." |
| **Total** | **~5.5 min** | | |
...
| P4.5 | All four Dark Graph types are demonstrated with distinct evidence | Demo shows Type 1 (telemetry), Type 2 (behaviour), Type 3 (fragment), Type 4 (document archaeology) evidence separately |
...
| P4.14 | Identity fragmentation cluster displays ≥8 dark identifiers for aeriella | Demo shows: 9 identifiers extracted from documents, 1 in CMDB, 88.9% handover loss |
| P4.15 | Cross-lifecycle link between 3G and 5G identifiers is visible | Demo shows: 3G-SITE-3847 → 5G-NR-3847 with decommission risk warning |
| P4.16 | Identity alias integration writes aliases to Datagerry | `pedkai integrate --fragment F1 --action register-aliases` → verify aliases exist in Datagerry REST API |
...
-- Phase 1: Deployment documents (Type 4 handover dark graph)
CREATE TABLE deployment_documents (
    document_id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    tenant_id TEXT NOT NULL,
    instance_id TEXT NOT NULL,
    document_type TEXT NOT NULL,     -- 'survey_report', 'h_and_s_assessment', 'method_statement', etc.
    authoring_party TEXT NOT NULL,   -- 'surveyor', 'radio_planner', 'contractor', etc.
    party_identifier TEXT NOT NULL,  -- The identifier THIS party uses for the site
    other_identifiers JSONB,         -- Other identifiers mentioned in this document
    canonical_identifier TEXT,       -- Ground truth CMDB hostname (for test validation)
    content_text TEXT,               -- Synthetic unstructured text body
    embedded_entities JSONB,         -- Entities mentioned: [{name, type, context}]
    geographic_coordinates JSONB,    -- {lat, lon, format} when applicable
    cross_lifecycle_refs JSONB,      -- References to older technology-era identifiers
    created_at TIMESTAMPTZ DEFAULT NOW(),
    
    CONSTRAINT idx_deploy_doc_instance CREATE INDEX ON deployment_documents(instance_id),
    CONSTRAINT idx_deploy_doc_party CREATE INDEX ON deployment_documents(authoring_party)
);

-- Phase 1: Identity fragment clusters (Type 4 pre-hypothesis)
CREATE TABLE identity_fragment_clusters (
    cluster_id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    tenant_id TEXT NOT NULL,
    canonical_cmdb_id TEXT,          -- CMDB entity if one exists in the cluster (may be NULL)
    alias_identifiers JSONB NOT NULL, -- {party_role: identifier}
    geographic_centroid JSONB,       -- {lat, lon}
    document_provenance JSONB NOT NULL, -- {doc_id: [identifiers found]}
    cluster_cohesion FLOAT DEFAULT 0,
    cmdb_coverage FLOAT DEFAULT 0,   -- Fraction of identifiers known to CMDB
    handover_loss FLOAT DEFAULT 0,   -- 1 - cmdb_coverage
    created_at TIMESTAMPTZ DEFAULT NOW()
);

-- Phase 1: Cross-lifecycle identifier links
CREATE TABLE cross_lifecycle_links (
    link_id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    tenant_id TEXT NOT NULL,
    cluster_id UUID REFERENCES identity_fragment_clusters(cluster_id),
    old_identifier TEXT NOT NULL,    -- e.g., '3G-SITE-3847'
    new_identifier TEXT NOT NULL,    -- e.g., '5G-NR-3847'
    old_technology_era TEXT,         -- '2G', '3G', '4G'
    new_technology_era TEXT,         -- '4G', '5G', 'FTTX'
    evidence_document_id UUID REFERENCES deployment_documents(document_id),
    confidence FLOAT DEFAULT 0,
    decommission_risk BOOLEAN DEFAULT FALSE,  -- True if old-era decommission could impact new-era
    created_at TIMESTAMPTZ DEFAULT NOW()
);

-- Phase 2: Edge hypotheses (extended for four Dark Graph types)
...
-- Phase 2: Identity fragmentation hypotheses (Type 4 multi-party handover)
CREATE TABLE identity_fragmentation_hypotheses (
    hypothesis_id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    tenant_id TEXT NOT NULL,
    canonical_entity_id TEXT,              -- CMDB entity, if one exists (may be NULL for dark sites)
    alias_identifiers JSONB NOT NULL,      -- {party_role: identifier}
    state TEXT NOT NULL DEFAULT 'candidate',
    confidence FLOAT NOT NULL DEFAULT 0.0,
    confidence_components JSONB,
    evidence_bundle JSONB NOT NULL DEFAULT '[]',
    geographic_proximity FLOAT DEFAULT 0,
    document_co_occurrence FLOAT DEFAULT 0,
    party_linkage FLOAT DEFAULT 0,
    temporal_overlap FLOAT DEFAULT 0,
    naming_pattern_coherence FLOAT DEFAULT 0,
    cross_lifecycle_links JSONB DEFAULT '[]',
    technology_eras JSONB DEFAULT '[]',    -- ['3G', '4G', '5G']
    cmdb_coverage FLOAT DEFAULT 0,
    handover_loss FLOAT DEFAULT 0,
    total_source_documents INT DEFAULT 0,
    dark_graph_types JSONB DEFAULT '["handover"]',
    policy_decision TEXT,                  -- 'register_aliases' | 'merge_identifiers' | 'link_lifecycle' | 'review'
    created_at TIMESTAMPTZ DEFAULT NOW(),
    last_evidence_at TIMESTAMPTZ,
    state_transitions JSONB DEFAULT '[]',
    
    CONSTRAINT idx_frag_state CREATE INDEX ON identity_fragmentation_hypotheses(state),
    CONSTRAINT idx_frag_canonical CREATE INDEX ON identity_fragmentation_hypotheses(canonical_entity_id)
);

-- Phase 2: Abeyance memory (extended TTL: Type 1=30d, Type 2=90d, Type 3=365d, Type 4=730d)
CREATE TABLE abeyance_items (
    item_id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    evidence JSONB NOT NULL,
    unresolved_entity TEXT NOT NULL,
    resolution_hints JSONB NOT NULL,
    dark_graph_type TEXT,           -- Determines TTL: Type 1=30d, Type 2=90d, Type 3=365d, Type 4=730d
    created_at TIMESTAMPTZ DEFAULT NOW(),
...
backend/app/adapters/
    ...
    synthetic_deployment_doc_generator.py  # Phase 1: Type 4 multi-party deployment document synthesis
...
backend/app/services/
    ...
    divergence_service.py         # Phase 1: Divergence report generation (nodes + edges + fragments)
    ...
    identity_resolution_service.py # Phase 1: Type 4 identifier extraction, clustering, and cross-lifecycle link detection
    deployment_document_parser.py  # Phase 1: Type 4 NER + regex identifier extraction from unstructured text
    hypothesis_engine.py          # Phase 2: Edge + Node + Mutation + Fragmentation hypothesis lifecycle
    node_hypothesis_engine.py     # Phase 2: NodeHypothesis + IdentityMutationHypothesis corroboration
    fragmentation_hypothesis_engine.py # Phase 2: IdentityFragmentationHypothesis corroboration
    corroboration_service.py      # Phase 2: Multi-modal noisy-OR scoring (edges + nodes + fragments)
    abeyance_memory.py            # Phase 2: Delayed evidence resolution (type-aware TTL: 30/90/365/730d)
...
backend/app/data/
    ...
    party_identifier_schemes.yaml # Phase 1: Type 4 per-party naming convention definitions
    deployment_document_templates.yaml # Phase 1: Type 4 synthetic document content templates
    cross_lifecycle_config.yaml   # Phase 1: Type 4 technology-era identifier mapping rules

backend/app/cli/
    pedkai_cli.py                 # Phase 4: CLI interface for demo

backend/app/policies/
    global_policies.yaml          # Extended with data_integrity + secure_ci + operator_behaviour + handover policies
```
